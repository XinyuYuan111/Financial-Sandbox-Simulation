    ...
    <div className="quickstart-grid">
      ...
      <section className="preview-panel">
        ...
        {preview ? <>
          ...
          <div className="preview-table table-scroll"><table><thead><tr><th>Agent</th><th>资金画像</th><th>规划器</th><th>角色</th></tr></thead><tbody>{preview.agent_definitions.slice(0, 8).map(agent => <tr key={agent.agent_id}><td><strong>{agent.display_name}</strong><small>{agent.agent_id}</small></td><td>{agent.funding_profile}</td><td>{agent.planner_profile_id}</td><td>{agent.role_tags.join(', ') || '-'}</td></tr>)}</tbody></table>{preview.agent_definitions.length > 8 ? <div className="table-more">另有 {preview.agent_definitions.length - 8} 个 Agent</div> : null}</div>
