...
export function MarketWorkspace({ projection }: { projection: Projection }) {
  ...
  useEffect(() => { prevPriceRef.current = lastPrice }, [lastPrice])
  const { flashingTradeIds, priceDirection, particles } = useTradeEffects(trades, lastPrice, prevPrice)
  return <div className="market-workspace" style={{ position: 'relative' }}>
    <ParticleLayer particles={particles} />
    <section className="metric-strip">
      <div><span>最新成交</span><strong className={priceDirection === 'up' ? 'price-up' : priceDirection === 'down' ? 'price-down' : ''}>{last?.price ?? '-'}</strong><small>USDx ticks</small></div>
    ...
    <div className="market-grid">
      <section className="workspace-panel orderbook-panel glass-shimmer">
